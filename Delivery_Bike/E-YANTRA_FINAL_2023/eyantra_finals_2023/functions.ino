...
void  whitelinefollow(float na,float nb,float nc,float nd,float ne, int align, float kp, float ki, float kd,int dl, bool bhool)
{ 
...
if(bhool || (last_error>0 && error<=0 || error>0 && last_error<=0) || (dl==0 && cs=='S')){integral=0;}

error=proportional * kp + integral * ki + derivative * kd;

 if(na+nb+nc+nd+ne>=3600){error=0;}
